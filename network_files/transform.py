...
class GeneralizedRCNNTransform(nn.Module):
    ...
    def resize(self, image, target):
        ...
        # print("pre " + str(image.shape))
        # import cv2, numpy
        # resultFile = "../visual/bbox/"
        # print(type(image))
        # image_id = 1
        # for n in range(len(target["boxes"])):
        #     x, y, w, h = target["boxes"][n]
        #     x, y, w, h = int(x), int(y), int(w), int(h)
        #     cv2.rectangle(image.cpu().numpy(), (x, y), (x + w, y + h), (0, 0, 255))
        # cv2.imwrite(resultFile + str(image_id) + 'before.png', image.cpu().numpy())
        # print("生成图片存在{}".format(resultFile))

        image, target = _resize_image_and_masks(image, size, float(self.max_size), target, self.fixed_size)

        # print("post " + str(image.shape))
        # for n in range(len(target["boxes"])):
        #     x, y, w, h = target["boxes"][n]
        #     x, y, w, h = int(x), int(y), int(w), int(h)
        #     cv2.rectangle(image.cpu().numpy(), (x, y), (x + w, y + h), (0, 0, 255))
        # cv2.imwrite(resultFile + str(image_id) + 'after.png', image.cpu().numpy())
        # print("生成图片存在{}".format(resultFile))
        ...
        bbox = target["boxes"]
        # bbox_fcos = target["boxes_fcos"]
        # 根据图像的缩放比例来缩放bbox
        bbox = resize_boxes(bbox, [h, w], image.shape[-2:])
        # bbox_fcos = resize_boxes(bbox_fcos, [h, w], image.shape[-2:])
        target["boxes"] = bbox
        # target["boxes_fcos"] = bbox_fcos
